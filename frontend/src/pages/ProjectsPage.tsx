  ...
  return (
    <div className="p-6 space-y-4">
      ...
      <Input label="Project Name *" value={name} onChange={e => setName(e.target.value)}
        placeholder="e.g. Sales DW Pipeline" autoFocus />
      ...
      <div className="max-w-5xl mx-auto space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-[#8b949e] mt-1">Organize your ETL pipelines</p>
          </div>
        ...
        {loading ? (
          ...
          <Card>
            <EmptyState
              icon={<svg fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M3 7v10a2 2 0 002 2h14a2 2 0 002-2V9a2 2 0 00-2-2h-6l-2-2H5a2 2 0 00-2 2z" /></svg>}
              title="No projects yet"
              description="Create a project to organize your ETL jobs"
              action={<Button variant="primary" size="sm" onClick={() => setShowForm(true)}>Create Project</Button>}
            />
          </Card>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {projects.map(p => (
              <Card key={p.id} className="p-5 group" onClick={() => navigate(`/projects/${p.id}`)}>
                <div className="flex items-start justify-between mb-3">
                  <div className="w-9 h-9 rounded-lg bg-gradient-to-br from-[#1f3d6e] to-[#1a3050]
                  ...
                  <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                    <button
                      ...
                      className="p-1.5 rounded text-[#8b949e] hover:text-[#e6edf3] hover:bg-[#252d3d] transition-colors">
                    ...
                    <button
                      ...
                      className="p-1.5 rounded text-[#8b949e] hover:text-[#f85149] hover:bg-[#2d0f0f] transition-colors">
                      {deleting === p.id ? <Spinner size="sm" /> : (
                        <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2}
                ...
                {p.description && (
                  <p className="text-xs text-[#8b949e] mt-1 line-clamp-2">{p.description}</p>
                )}
                <div className="mt-3 pt-3 border-t border-[#21262d] flex items-center justify-between">
                  <span className="text-xs text-[#484f58]">
                    Updated {new Date(p.updatedAt).toLocaleDateString()}
                  </span>
                  <svg className="w-3.5 h-3.5 text-[#484f58] group-hover:text-[#58a6ff] transition-colors"
