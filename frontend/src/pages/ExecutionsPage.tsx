...
function statusVariant(
  s: ExecutionStatus,
): "success" | "error" | "warning" | "blue" | "default" {
  switch (s) {
    case "SUCCESS":
      return "success";
    case "FAILED":
            ...
            <Card>
              <div className="flex flex-col items-center justify-center py-16 gap-4">
                ...
                <div className="text-center">
                  ...
                  <p className="text-xs mt-1" style={{ color: "#3d5573" }}>
                    Run a job from the Job Designer to see results here
                  </p>
                </div>
              </div>
            </Card>
    ...
    <div className="space-y-4">
        ...
        <div className="flex items-start justify-between mb-4">
          <div>
            <div className="flex items-center gap-2 mb-1">
              ...
              <span className="text-xs font-mono" style={{ color: "#3d5573" }}>
                {result.executionId}
              </span>
            </div>
                ...
                >
                  <div className="flex items-center gap-2.5">
                    ...
                    <div>
                      ...
                      {r.errorMessage && (
                        <p className="text-xs" style={{ color: "#f87070" }}>
                          {r.errorMessage}
                        </p>
                      )}
                    </div>
                  ...
                  >
                    {r.rowsProcessed > 0 && (
                      <span>{r.rowsProcessed.toLocaleString()} rows</span>
                    )}
                    {r.rowsRejected > 0 && (
                      <span style={{ color: "#fbbf24" }}>
                        {r.rowsRejected} rejected
                      </span>
                    )}
      ...
      {result.logs.length > 0 && (
        <Card className="p-5">
          ...
          <div
            className="rounded-lg p-4 font-mono text-xs space-y-1 max-h-80 overflow-y-auto"
      ...
      {Object.values(result.nodeResults).some((r) => r.generatedSql) && (
        <Card className="p-5">
          ...
          <div className="space-y-3">
            {Object.entries(result.nodeResults)
              .filter(([, r]) => r.generatedSql)
              .map(([id, r]) => (
                <div key={id}>
                  <p className="text-xs mb-1" style={{ color: "#3d5573" }}>
                    {r.nodeType}
                  </p>
                  <pre
                    className="rounded-lg p-3 text-xs font-mono overflow-x-auto whitespace-pre-wrap"
                    style={{
                      background: "#06101c",
