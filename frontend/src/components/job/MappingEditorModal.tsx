...
import { type MappingRow, getAutoExpression, buildAutoMappings } from '../../utils/mapping'
...
const ROW_COLORS = [
  '#58a6ff', '#3fb950', '#bc8cff', '#f0883e', '#d29922', '#f85149',
...
export default function MappingEditorModal({
  ...
  useEffect(() => {
    ...
    groups.forEach((g, idx) => {
      ...
      if (cachedCols && cachedCols.length > 0) {
        ...
        setSourceGroups(prev => prev.map((p, i) =>
      ...
      if (!g.connectionId || !g.tableName) {
        setSourceGroups(prev => prev.map((p, i) => i === idx ? { ...p, loading: false } : p))
        return
      }
  ...
  const handleSourceClick = (nodeId: string, colName: string) => {
    if (selectedSourceCol?.nodeId === nodeId && selectedSourceCol?.col === colName) {
      setSelectedSourceCol(null)
    } else {
  ...
  const handleAddRow = () => {
    if (selectedSourceCol) {
      const existingGroup = sourceGroups.find(g => g.nodeId === selectedSourceCol.nodeId)
      const col = existingGroup?.columns.find(c => c.columnName === selectedSourceCol.col)
  ...
  const handleTargetClick = (rowId: string) => {
    ...
    setMappings(prev => prev.map(m => m.id === rowId
  ...
  const updateMapping = (id: string, key: keyof MappingRow, value: string) => {
    setMappings(prev => prev.map(m => m.id === id ? { ...m, [key]: value } : m))
  }
  ...
  const getSourceColor = (nodeId: string) => {
    return sourceGroups.find(g => g.nodeId === nodeId)?.color ?? '#8b949e'
  }

  const isMapped = (nodeId: string, colName: string) =>
    mappings.some(m => m.sourceNodeId === nodeId && m.sourceColumn === colName)
  ...
  const downloadSample = () => {
    const sample = mappings.length > 0
      ? mappings.map(({ id, ...rest }) => rest)
      : [{ sourceNodeId: '', sourceColumn: 'COL_A', targetName: 'col_a', expression: '', type: 'VARCHAR' }]
    const blob = new Blob([JSON.stringify(sample, null, 2)], { type: 'application/json' })
    const url = URL.createObjectURL(blob)
    const a = document.createElement('a')
    a.href = url; a.download = 'mapping.json'; a.click()
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-5xl h-[80vh] bg-[#161b27] border border-[#30363d]
        ...
        <div className="flex items-center justify-between px-5 py-3.5 border-b border-[#30363d] flex-shrink-0">
          <div className="flex items-center gap-3">
            <div className="w-7 h-7 rounded bg-[#0d1f35] flex items-center justify-center">
            ...
            <div>
              <p className="text-sm font-semibold text-[#e6edf3]">Mapping Editor — {nodeLabel}</p>
          ...
          <div className="flex items-center gap-2">
            <button onClick={downloadSample}
              className="flex items-center gap-1.5 px-3 py-1.5 text-xs text-[#8b949e]
                hover:text-[#e6edf3] bg-[#252d3d] rounded-md border border-[#30363d] transition-colors">
            ...
            <button onClick={onClose}
              className="p-1.5 rounded text-[#484f58] hover:text-[#e6edf3] hover:bg-[#252d3d] transition-colors">
        ...
        <div className="flex flex-1 min-h-0">
          ...
          <div className="w-[260px] flex-shrink-0 border-r border-[#21262d] flex flex-col">
            <div className="px-3 py-2 border-b border-[#21262d] bg-[#0d1117]">
              <p className="text-xs font-semibold text-[#8b949e] uppercase tracking-wider">Source Columns</p>
            </div>
            <div className="flex-1 overflow-y-auto">
              ...
              {sourceGroups.length === 0 && loadingCount === 0 && (
                <div className="p-4 text-xs text-[#484f58] text-center">
                  연결된 Input 노드가 없습니다
                </div>
              )}
              {sourceGroups.map(group => (
                <div key={group.nodeId}>
                  {/* Node header */}
                  <div className="px-3 py-2 bg-[#1c2333] border-b border-[#21262d]"
                    style={{ borderLeft: `3px solid ${group.color}` }}>
                    <p className="text-xs font-semibold truncate" style={{ color: group.color }}>
                      {group.nodeLabel}
                    </p>
                    <p className="text-[10px] text-[#484f58] truncate">{group.tableName || '(테이블 미설정)'}</p>
                  </div>
                  {group.loading ? (
                    <div className="flex justify-center py-3"><Spinner size="sm" /></div>
                  ) : group.columns.length === 0 ? (
                    <div className="px-3 py-2 text-xs text-[#484f58]">컬럼 정보 없음</div>
                  ) : (
                    group.columns.map(col => {
                      const isSelected = selectedSourceCol?.nodeId === group.nodeId
                      ...
                      return (
                        <div
                          ...
                          className={`flex items-center gap-2 px-3 py-1.5 cursor-pointer
                            border-b border-[#21262d] transition-colors select-none
                            ${isSelected
                              ? 'bg-[#1f3d6e]'
                              : mapped
                        ...
                        >
                          <span className="w-1.5 h-1.5 rounded-full flex-shrink-0"
                            style={{ backgroundColor: mapped ? group.color : '#30363d' }} />
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-1.5">
                              {col.isPrimaryKey && (
                                <svg className="w-2.5 h-2.5 text-[#d29922] flex-shrink-0"
                              ...
                              <span className="text-xs text-[#c9d1d9] truncate font-mono">{col.columnName}</span>
                            </div>
                            <span className="text-[10px] text-[#484f58]">{col.dataType}</span>
                          </div>
                          {isSelected && (
                            <svg className="w-3 h-3 text-[#58a6ff] flex-shrink-0"
          ...
          <div className="flex-1 flex flex-col min-w-0">
            <div className="flex items-center justify-between px-4 py-2 border-b border-[#21262d] bg-[#0d1117] flex-shrink-0">
              <p className="text-xs font-semibold text-[#8b949e] uppercase tracking-wider">Target Mappings</p>
              <div className="flex items-center gap-2">
                <button onClick={handleAutoMap}
                  className="flex items-center gap-1.5 px-2.5 py-1 text-xs text-[#58a6ff]
                    bg-[#0d1f35] border border-[#1a3050] rounded hover:bg-[#1a3050] transition-colors">
                ...
                <button onClick={() => setMappings([])}
                  className="px-2.5 py-1 text-xs text-[#8b949e] hover:text-[#f85149]
                    bg-[#252d3d] rounded border border-[#30363d] transition-colors">
                  Clear All
                </button>
              </div>
            </div>
            ...
            {/* Column Headers */}
            <div className="grid grid-cols-[28px_1fr_1fr_1fr_80px_32px] gap-0 border-b border-[#21262d]
              px-4 py-1.5 bg-[#1c2333] text-[10px] font-semibold text-[#484f58] uppercase tracking-wider flex-shrink-0">
              <div />
            ...
            <div className="flex-1 overflow-y-auto">
              {mappings.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-12 gap-3">
                  <div className="w-10 h-10 rounded-lg bg-[#252d3d] flex items-center justify-center">
                    <svg className="w-5 h-5 text-[#484f58]" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5}
                        d="M8 7h12m0 0l-4-4m4 4l-4 4m0 6H4m0 0l4 4m-4-4l4-4" />
                    </svg>
                  </div>
                  <p className="text-xs text-[#484f58]">소스 컬럼을 선택하거나 Auto Map을 클릭하세요</p>
                </div>
              ) : (
                mappings.map((m, idx) => {
                  ...
                  return (
                    <div
                      ...
                      className={`grid grid-cols-[28px_1fr_1fr_1fr_80px_32px] gap-0 items-center
                        border-b border-[#21262d] px-4 py-1.5 group transition-colors
                        ${isClickTarget ? 'cursor-pointer hover:bg-[#1f3d6e]' : 'hover:bg-[#1c2333]'}`}
                    >
                      {/* Color dot + index */}
                      ...
                      <div className="pr-2">
                        <select
                          ...
                          className="w-full bg-transparent text-xs text-[#c9d1d9] font-mono
                      ...
                      <div className="pr-2">
                        <input
                          ...
                          className="w-full bg-transparent text-xs text-[#e6edf3] font-mono
                            focus:outline-none border-b border-transparent focus:border-[#58a6ff] py-0.5"
                        />
                      </div>
                      ...
                      <div className="pr-2">
                        <input
                          ...
                          className="w-full bg-transparent text-xs text-[#bc8cff] font-mono
                            focus:outline-none border-b border-transparent focus:border-[#58a6ff] py-0.5"
                        />
                      </div>
                      ...
                      <div className="pr-2">
                        <select
                          ...
                          onClick={e => e.stopPropagation()}
                          className="w-full bg-transparent text-xs text-[#8b949e]
                            focus:outline-none border-0 cursor-pointer"
                        >
                      ...
                      <div className="flex justify-center">
                        <button
                          ...
                          className="opacity-0 group-hover:opacity-100 p-1 rounded
                            text-[#484f58] hover:text-[#f85149] hover:bg-[#2d0f0f] transition-all">
                          <svg className="w-3 h-3" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2}
              ...
              <button
                ...
                className="flex items-center gap-2 w-full px-4 py-2.5 text-xs text-[#484f58]
                  hover:text-[#58a6ff] hover:bg-[#0d1f35] transition-colors border-t border-[#21262d]">
                <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 4v16m8-8H4" />
                </svg>
                {selectedSourceCol ? `"${selectedSourceCol.col}" 매핑 행 추가` : '행 추가'}
              </button>
            </div>
          </div>
        </div>
        ...
        {/* Footer */}
        <div className="flex items-center justify-between px-5 py-3 border-t border-[#30363d] flex-shrink-0
          bg-[#0d1117]">
          <p className="text-xs text-[#484f58]">
          ...
          <div className="flex items-center gap-2">
            ...
            <button onClick={handleApply}
              className="px-5 py-2 text-sm font-medium rounded-md
                bg-[#238636] hover:bg-[#2ea043] text-white transition-colors">
              Apply
            </button>
          </div>
        </div>
